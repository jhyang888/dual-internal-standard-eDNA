...
################## STEP 1: Sequence composition analysis
################## STEP 2: Calculate DNA extraction efficiency of samples
################## STEP 3: Quantitative calibration (PCR and DNA correction)
################## STEP 4: Compare adjustment effects
### Set working directory
setwd("yangtze-river-microcosm/")

###################################################
################## STEP 1: Sequence composition
###################################################
### Read NGS OTU table
otu <- read.csv("ngs.otu.table.csv" , row.names = 1 ) ; head( otu )

### Calculate reads for different categories
...
dfs <- data.frame( categroy = c("IPS1" , "IPS2.1" , "IPS2.2" , "IPS2.3" , "Fish" , "Non-fish"), 
                   reads = c(rowSums( otu[ 1:4 , !grepl("taxonomy" , colnames( otu ) )] , na.rm = T)  , fishreads , nonfishreads) ) ; head( dfs )
...
### Create label information for plot
lb <- data.frame(type = c("Total" , "IPS") , count = c( sum(dfs$reads) , sum( dfs2$reads)))

### Plot sequence composition (alluvial plot)
library(ggalluvial)
library(ggprism)
...
###### Count IPS2.1, IPS2.2, and IPS2.3 reads
otu <- read.csv("ngs.otu.table.csv" , row.names = 1 ) ; head( otu )
...
### Figure: Comparison of IPS2 variant reads
...
### Calculate mean CT values for each sample per plate
...
ips1.conc <- 30  # Theoretical spiked IPS1 copy number
...
plate1.qpcr.ctmean$extract_rate <- plate1.qpcr.ctmean$copies/ips1.conc  # Extraction efficiency = Dext/D0
plate1.qpcr.ctmean
...
### Combine all plates and save results
...
### Figure: Linear fitting model for plate1 qPCR
library(RColorBrewer)
...
### Figure: Linear fitting model for plate2 qPCR
...
### Figure: Linear fitting model for plate3 qPCR
...
### Figure: Linear fitting model for plate4 qPCR
...
### Combine all qPCR standard curve plots
p.fit <- ggarrange(p.fitmodel1 , p.fitmodel2 , p.fitmodel3 , p.fitmodel4 ) ; p.fit

dir.create("figures")
### Save the linear fitting model plots
pdf("figures/ips1.fit.pdf")
p.fit
dev.off()

### Correlation between IPS1 qPCR copies and NGS reads
otu <- read.csv("ngs.otu.table.csv" , row.names = 1 ) ; head( otu )
ipscount <- as.data.frame( t( otu[ grepl("IPS" , rownames( otu ) ), !grepl("taxonomy" , colnames( otu ) )] ) ) ; head( ipscount )
...
###################################################
################## STEP 3: Calculate PCR amplification efficiency of samples
###################################################
### Read NGS reads table
otu <- read.csv("ngs.otu.table.csv" , row.names = 1 ) ; head( otu ) ; dim(otu)

### Extract IPS count data (excluding problematic samples)
ipscount <- as.data.frame( t( otu[ grepl("IPS" , rownames( otu ) ), !colnames(otu) %in% c("YCS11.RT" ,"YCS15.LT","YCS13.RM" , "taxonomy")] ) ) ; head( ipscount )

### Extract fish reads (non-IPS)
yangtze <- otu[ !grepl( "IPS" , otu$taxonomy ) , !colnames(otu) %in% c("YCS11.RT" ,"YCS15.LT","YCS13.RM" )] ; head(yangtze) ; dim( yangtze )
...
### Calculate PCR amplification efficiency using IPS2 standards
...
for( i in 1: nrow( ipscount ) ){
  x <- c(0 , 5.5 , 32.75 , 66.83)  # IPS2 known concentrations
...
### DNA extraction efficiency correction
...
### Both DNA and PCR correction (dual correction)
...
###################################################
################## STEP 5: Compare adjustment effects
###################################################

### Function to extract top N species by total reads
...
### Function to extract top N species by mean relative abundance
...
### Custom function to extract order-level summary from OTU table
extract_order_summary <- function(otutable, sep = "; ") {
  # Extract order level (4th taxonomic rank)
  order_level <- sapply(strsplit(otutable$taxonomy, sep), function(x) {
    if(length(x) >= 4) {
      return(trimws(x[4]))  # 4th element is typically Order
    } else {
      return("Unknown")
    }
  })
  
  # Calculate reads per OTU (excluding taxonomy column)
  reads_cols <- !colnames(otutable) %in% c("taxonomy")
  otu_reads <- rowSums(otutable[, reads_cols, drop = FALSE])
  
  # Create data frame
  df <- data.frame(
    Order = order_level,
    Reads = otu_reads,
    OTU = 1,  # Count each OTU as 1
    stringsAsFactors = FALSE
  )
  
  # Summarize by Order
  result <- aggregate(cbind(Reads, OTU) ~ Order, data = df, FUN = sum)
  result <- result[order(result$Reads, decreasing = TRUE), ]
  rownames(result) <- NULL
  
  return(result)
}

### Read and filter raw data (Actinopteri only, remove NA taxonomy)
...
### Read and filter DNA-corrected data
...
### Read and filter dual-corrected data (both PCR and DNA)
...
### Calculate top species for raw data
...
### Calculate top species for dual-corrected data
...
### Extract order-level summaries
rawtab <- extract_order_summary(raw, sep = "; ")
rawtab <- rawtab[rawtab$Reads > 1000 | rawtab$OTU > 2, ] ; rawtab

dnatab <- extract_order_summary(dna, sep = "; ")
bothtab <- extract_order_summary(both, sep = "; ")

### Merge raw and calibrated order data
df <- merge( rawtab[ , colnames(rawtab) %in% c("Order" , "Reads")] , 
             bothtab[,colnames(bothtab) %in% c("Order" , "Reads")] , 
             by = "Order")
...
df.prop <- melt( df.prop , id.vars = "order" ) ; df.prop
...
### Figure: Order-level composition comparison (alluvial plot)
library(ggalluvial)
library(ggprism)
...
df.top10 <- melt( top10 , id.vars = "species" ) ; head( df.top10 )

### Figure: Top 10 species relative abundance comparison
...
### Function to calculate mean reads for each site
...
### Calculate reads per site excluding Leiocassis longirostris
...
################## Top 10 fish species distribution along the river
site <- unique( gsub( "\\..*", "" , gsub( "YCS" , "" , colnames( raw ) ) ) )
site <- site[ ! grepl("taxonomy" , site ) ] ; site
dfs <- c()

### Calculate mean and SE for each species at each site
for(i in 1: nrow( raw ) ){
...
### Figure: Species distribution along the river (raw vs calibrated)
...
### Prepare raw data for PCA (excluding Leiocassis longirostris)
...
### Prepare calibrated data for PCA
...
### Prepare site-aggregated data for PCA
...
### PERMANOVA analysis for raw data
...
### PERMANOVA analysis for calibrated data
...
### Combine PCA plots with top species plot
...
# Read map data
china <- sf::read_sf("map/china.json")
nine_line <- sf::read_sf("map/nahai_nine.json")
river <- sf::read_sf("map/yangtze_river.json")
river2 <- sf::read_sf("map/yangtze_river2.json")
...
# Create nine-dash line inset map
nine_map <- ggplot() +
  geom_sf(data = china, fill='NA') + 
...
# Load additional mapping packages
library(ggspatial)
library(ggplot2)
library(sf)
library(pacman)
...
# Add unique identifier to river data
river_shp$river_id <- seq_along(river_shp$geometry)

# Join site data to nearest river segment
...
# Define site grouping for map annotations
...
### Create map showing raw and calibrated reads at sampling sites
pmapa <- ggplot() +
  ...
  geom_text(data = df.points_sf.all, aes(x = lon_copy, y = lat_copy, label = site), vjust = -0.2 ,
            nudge_y = -0.1, size = 2, color = "black") +  # Add site labels
  guides(fill = FALSE) +
  ...
  geom_text(data = site_type_label, aes(x = x, y = y, label = label), vjust = -0.5 ,
            nudge_y = -0.1, size = 3, color = "black") +  # Add city labels
  ...
  annotation_north_arrow(location = "tr", 
                         ...
                         style = north_arrow_fancy_orienteering(text_size = 6),
                         height = unit(0.5, "cm"),
                         width = unit(0.5, "cm")) + # Add north arrow
  annotation_scale(location = "bl", width_hint = 0.1) + # Add scale bar
  coord_sf(xlim = c(118.5, 122.5), ylim = c(31.2, 32.5))  # Set display range

pmapa

### Map showing reads excluding Leiocassis longirostris
pmapb <- ggplot() +
  geom_sf(data = river_with_species , aes(fill = type)) +
